...
root = Tk()
root.resizable(0, 0)
root.title("Handwritten Digit Identification GUI App")
...
def Recognize_Digit():
    ...
    for cnt in contours:
        ...
        # numpy.argmax(input array) - Returns the indices of the max values
        final_pred = np.argmax(pred)
        data = str(final_pred) + ' ' + str(int(max(pred) * 100)) + '%'
        # cv2.putText() - This Method is used to draw a text string on image
